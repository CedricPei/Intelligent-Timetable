...
int input_method(int x,int y,char *str,int value,char *py)
{
	...
	char *ABpath="C:\\ckeshe\\pinyin\\";//汉语拼音检索标准路径
	char pypath[45]={"C:\\ckeshe\\pinyin\\"};//汉语拼音检索相对路径
	//settextjustify(LEFT_TEXT,CENTER_TEXT);
	while(1)
	{
		if(trigger||kbhit())//第一次进入自动触发 以后均需键盘
		{
			...
			settextstyle(1,0,2);
			outtextxy(PyStartx,PyStarty-7,py);        //拼音字体
		    strcat(pypath,py);
		    strcat(pypath,".txt");
...
int xouttextxy(int x,int y,char *s,int color)//8x16点阵字库
{
	...
	if((asc=fopen("C:\\ckeshe\\HZK1\\ASC16","rb"))==NULL)
	{
		closegraph();
